...
void App_Init(void) {
  ...
  const string_view_t startup_msg = make_string_view(
      "=== STM32 Button & Encoder Test Demo (C++23 OOP) ===\r\n");
  const string_view_t success_msg = make_string_view(
      "Button & Encoder Test initialized successfully!\r\n\r\n");
  ...
  if (BootAnimation_Init(&u8g2)) {
    serial_printf("Boot animation initialized successfully\r\n");

    // 启动开机动画
    if (BootAnimation_Start()) {
      serial_printf("Boot animation started\r\n");
  ...
  serial_printf("Encoder features: Acceleration enabled, Button integrated, "
                "Polling Mode\r\n");
  ...
  serial_printf(
      "PWM Frequency: ~1.1kHz, Resolution: 65536 levels (16-bit)\r\n");
...
static void draw_button_encoder_test(void) {
  ...
  do {
    ...
    snprintf(button_stats, sizeof(button_stats), "P:%u C:%u L:%u M:%u",
             (unsigned int)button_press_count, (unsigned int)button_click_count,
             (unsigned int)button_long_press_count,
             (unsigned int)button_multi_click_count);
    ...
    const char *speed_str = (current_encoder_speed == ENCODER_SPEED_FAST) ? "F"
                            : (current_encoder_speed == ENCODER_SPEED_MEDIUM)
                                ? "M"
                                : "S";
