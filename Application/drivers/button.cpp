...
void Button::init() {
    ...
    serial_printf("Button: Initialized on Port %p Pin %u (Active %s)\r\n",
                  port_, pin_, active_low_ ? "Low" : "High");
}
 ...
 */
void Button::handleMultiClick(uint8_t max_clicks, MultiClickCallback callback, uint32_t gap_ms) {
 ...
 */
ButtonState_t Button::getState() const {
    ...
    if (interrupt_mode_enabled_) {
        ...
        if (current_time - last_change_time_ > BUTTON_DEBOUNCE_TIME_MS) {
            bool current_pressed = readPin();
                ...
                ButtonState_t new_state = current_pressed ? BUTTON_STATE_PRESSED : BUTTON_STATE_RELEASED;
...
void Button::stateMachine() {
    ...
    ButtonState_t new_state = current_pin_state ? BUTTON_STATE_PRESSED : BUTTON_STATE_RELEASED;
    ...
    if (long_press_enabled_ && current_state_ == BUTTON_STATE_PRESSED && !long_press_triggered_) {
        uint32_t press_duration = current_time - press_start_time_;
        if (press_duration >= long_press_time_ms_) {
