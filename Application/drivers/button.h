...
public:
  ...
  using EventCallback =
      std::function<void(ButtonEvent_t event, ButtonState_t state)>;
...
public:
   ...
   */
  void handleMultiClick(uint8_t max_clicks, MultiClickCallback callback,
                        uint32_t gap_ms = BUTTON_DEFAULT_MULTI_CLICK_GAP_MS);
